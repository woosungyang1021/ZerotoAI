    ...
    for c in CASES:
        ...
        while True:
            ...
            for name in ("ok", "bad", "wait"):
                if name == "wait":
                    m, side = white_line_mask(hsv)
                    segs = bottom_edges(m, side, min_w=100, min_side=45)
                else:
                    lo, hi = HSV[name]
                ...
                for sx0, sx1, sy in segs:
                    v = H @ np.array([(sx0 + sx1) / 2, sy, 1.0]); v = v / v[2]
                    gx, gy = float(v[0]), float(v[1])
                    if not (RANGE_X[0] < gx < RANGE_X[1] and RANGE_Y[0] < gy < RANGE_Y[1]):
                        continue
